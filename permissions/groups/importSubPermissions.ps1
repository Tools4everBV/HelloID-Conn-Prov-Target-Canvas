####################################################################
# HelloID-Conn-Prov-Target-Canvas-ImportSubPermissions-Group
# PowerShell V2
####################################################################
...
$permissionReference = 'lessonGroupCode'
$permissionDisplayName = 'Lesgroep Code'
...
try {
    ...
    foreach ($importedPermission in $retrievedPermissions) {
        $permission = @{
            ...
            SubPermissionDisplayName = "$($importedPermission.name)"
        }
