#################################################################
# HelloID-Conn-Prov-Target-Canvas-RevokePermission-Group
# PowerShell V2
#################################################################
    ...
    switch ($lifecycleProcess) {
        'RevokePermission' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Revoking Canvas permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"

                $splatParams = @{
                    Uri     = "$($Actioncontext.configuration.BaseUrl)/api/v1/groups/$($actionContext.References.Permission.Reference)/users/$($actionContext.References.Account)"
                    Method  = 'DELETE'
                    Headers = $headers
                }        
                ...
                Write-Information "[DryRun] Revoke Canvas permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }
        ...
        'NotFound' {
            ...
            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Canvas account: [$($actionContext.References.Account)] could not be found, indicating that it may have been deleted. Action initiated by: [$($actionContext.Origin)]"
                    IsError = $false
                })
            break
        }
