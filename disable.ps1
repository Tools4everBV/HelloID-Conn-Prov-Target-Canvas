...
try {
    ...
    if ($dryRun -eq $true) {
        $auditLogs.Add([PSCustomObject]@{
                Message = "Disable Canvas account for: [$($p.DisplayName)] will be executed during enforcement"
            })
    }
    ...
    if (-not($dryRun -eq $true)) {
        ...
        Write-Verbose "Disabling Canvas account with accountReference: [$aRef]"
        $splatParams = @{
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $errorMessage = "Could not disable Canvas account. Error: $($errorObj.ErrorMessage)"
    } else {
        $errorMessage = "Could not disable Canvas account. Error: $($ex.Exception.Message)"
    }
